...
bool CRenderBuffer::UploadBuffer()
{
  ...
  case BUFFER_FMT_D3D11_BYPASS:
  {
    const auto buf = static_cast<DXVA::COutputBuffer*>(videoBuffer);
...
ID3D11View* CRenderBuffer::GetView(unsigned idx)
{
  ...
  case BUFFER_FMT_D3D11_BYPASS:
  {
    ...
    auto dxva = dynamic_cast<DXVA::COutputBuffer*>(videoBuffer);
    if (!dxva)
      return nullptr;
...
{
  const auto dxva_buffer = dynamic_cast<DXVA::COutputBuffer*>(videoBuffer);
