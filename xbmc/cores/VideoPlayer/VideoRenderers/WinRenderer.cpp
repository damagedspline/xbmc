...
bool CWinRenderer::Configure(const VideoPicture &picture, float fps, unsigned int orientation)
{
  ...
  {
    auto* dxvaBuf = dynamic_cast<DXVA::COutputBuffer*>(picture.videoBuffer);
...
void CWinRenderer::UpdatePSVideoFilter()
{
  ...
  if (!m_colorShader->Create(m_bufferFormat, AVCOL_PRI_BT709, m_srcPrimaries, m_bUseHQScaler ? nullptr : m_outputShader.get()))
  {
    if (m_bUseHQScaler)
    {
      m_IntermediateTarget.Release();
