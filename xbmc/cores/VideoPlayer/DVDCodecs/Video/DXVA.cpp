...
// CDXVAOutputBuffer
//-----------------------------------------------------------------------------
COutputBuffer::COutputBuffer(int id)
    : CVideoBuffer(id)
{
...
void COutputBuffer::Initialize(CDecoder* decoder)
{
...
HRESULT COutputBuffer::GetResource(ID3D11Resource** ppResource)
{
  if (!view)
    return E_NOT_SET;
...
void COutputBuffer::SetRef(AVFrame* frame)
{
...
void COutputBuffer::Unref()
{
  view = nullptr;
  av_frame_unref(m_pFrame);
}

HRESULT COutputSharedBuffer::GetResource(ID3D11Resource** ppResource)
{
  HRESULT hr = S_OK;
...
void COutputSharedBuffer::Initialize(CDecoder* decoder)
{
  COutputBuffer::Initialize(decoder);
  handle = decoder->m_sharedHandle;
}

void COutputCopyBuffer::Initialize(CDecoder* decoder)
{
  COutputBuffer::Initialize(decoder);
  ...
  if (FAILED(COutputBuffer::GetResource(&pResource)))
  {
...
CBufferPool<_TBuf>::CBufferPool() = default;

template<typename _TBuf>
CBufferPool<_TBuf>::~CBufferPool()
{
...
template<typename _TBuf>
CVideoBuffer* CBufferPool<_TBuf>::Get()
{
  CSingleLock lock(m_section);

  COutputBuffer* retPic;
  if (!m_freeOut.empty())
  {
...
template<typename _TBuf>
void CBufferPool<_TBuf>::Return(int id)
{
  CSingleLock lock(m_section);
...
template<typename _TBuf>
void CBufferPool<_TBuf>::AddView(ID3D11View* view)
{
  CSingleLock lock(m_section);
...
template<typename _TBuf>
bool CBufferPool<_TBuf>::IsValid(ID3D11View* view)
{
  CSingleLock lock(m_section);
...
template<typename _TBuf>
bool CBufferPool<_TBuf>::ReturnView(ID3D11View* view)
{
  CSingleLock lock(m_section);
...
template<typename _TBuf>
ID3D11View* CBufferPool<_TBuf>::GetView()
{
  CSingleLock lock(m_section);
...
template<typename _TBuf>
bool CBufferPool<_TBuf>::HasFree()
{
  CSingleLock lock(m_section);
...
template<typename _TBuf>
bool CBufferPool<_TBuf>::HasRefs()
{
  CSingleLock lock(m_section);
...
bool CDecoder::Open(AVCodecContext* avctx, AVCodecContext* mainctx, enum AVPixelFormat fmt)
{
  ...
  if (m_dxvaContext->IsContextShared())
  {
    ...
    if (AIdentifier.VendorId == PCIV_Intel)
      m_bufferPool = std::reinterpret_pointer_cast<CBufferPool<COutputBuffer>>(std::make_shared<CBufferPool<COutputSharedBuffer>>());
    else
      m_bufferPool = std::reinterpret_pointer_cast<CBufferPool<COutputBuffer>>(std::make_shared<CBufferPool<COutputCopyBuffer>>());
  }
  else
    m_bufferPool = std::make_shared<CBufferPool<COutputBuffer>>();
...
CDVDVideoCodec::VCReturn CDecoder::Decode(AVCodecContext* avctx, AVFrame* frame)
{
  ...
  if (frame)
  {
    if (m_bufferPool->IsValid(reinterpret_cast<ID3D11View*>(frame->data[3])))
    {
      ...
      m_videoBuffer = reinterpret_cast<COutputBuffer*>(m_bufferPool->Get());
      if (!m_videoBuffer)
      {
...
bool CDecoder::CheckInternal() const
{
  ComPtr<ID3D11Device> pDevice;
  m_pD3D11Decoder->GetDevice(&pDevice);
...
bool CDecoder::OpenDecoder()
{
  ...
  m_avD3D11Context->surface_count = m_refs + m_shared; // refs + processor buffer
