...
bool CAESinkWASAPIWin10::Initialize(AEAudioFormat &format, std::string &device)
{
  ...
  if (!bdefault)
  {
    try
    {
      auto task = CAudioInterfaceActivator::ActivateAsync(localUtfToWide(device).c_str());
      task.then([&pClient](IAudioClient* client)
      {
        pClient = client;
      }).wait();
    }
  ...
  if (!pClient)
  {
    ...
    try
    {
      auto task = CAudioInterfaceActivator::ActivateAsync(deviceId->Data());
      task.then([&pClient](IAudioClient* client)
      {
        pClient = client;
      }).wait();
...
void CAESinkWASAPIWin10::EnumerateDevicesEx(AEDeviceInfoList &deviceInfoList, bool force)
{
  ...
  DeviceInformationCollection^ devInfocollection = nullptr;

  Concurrency::task<DeviceInformationCollection^> enumOperation(DeviceInformation::FindAllAsync(audioSelector, propertyList));
  enumOperation.then([&devInfocollection](DeviceInformationCollection^ list)
  {
    devInfocollection = list;
  }).wait();
  ...
  try
  {
    ...
    for (unsigned int i = 0; i < devInfocollection->Size; i++)
    {
      ...
      CAudioInterfaceActivator::ActivateAsync(devInfo->Id->Data())
      .then([&pClient](IAudioClient* client)
      {
        pClient = client;
      }).wait();
...
bool CAESinkWASAPIWin10::InitializeExclusive(AEAudioFormat &format)
{
  ...
  if (format.m_sampleRate == 0 ||
      format.m_channelLayout == nullptr ||
...
initialize:
  ...
  if (hr == AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED)
  {
    ...
    CAudioInterfaceActivator::ActivateAsync(deviceId->Data()).then([this](IAudioClient* client)
    {
