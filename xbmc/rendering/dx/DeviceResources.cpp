...
{
  static std::shared_ptr<DX::DeviceResources> sDeviceResources(new DX::DeviceResources);
...
bool DX::DeviceResources::SetFullScreen(bool fullscreen)
{
  ...
  bool result = true;
  auto handler = ref new DispatchedHandler([fullscreen, &result]()
  {
  ...
  if (m_window->Dispatcher->HasThreadAccess)
    handler->Invoke();
  else
  {
    auto async = m_window->Dispatcher->RunAsync(CoreDispatcherPriority::High, handler);
    Concurrency::create_task(async).wait();
  }
...
void DX::DeviceResources::CreateDeviceResources() 
{
  ...
  if (!m_adapter)
  {
    ...
    DX::ThrowIfFailed( adapter.As(&m_adapter) );

    DX::ThrowIfFailed(m_adapter->GetParent(IID_PPV_ARGS(&m_dxgiFactory)));
  }
}
...
void DX::DeviceResources::ShowCursor(bool show)
{
  if (!m_window.Get())
    return;

  auto handler = ref new DispatchedHandler([this, show]()
  {
    CoreCursor^ cursor = nullptr;
    if (show)
      cursor = ref new CoreCursor(CoreCursorType::Arrow, 1);
    m_window->PointerCursor = cursor;
  });

  if (m_window->Dispatcher->HasThreadAccess)
    handler->Invoke();
  else
    m_window->Dispatcher->RunAsync(CoreDispatcherPriority::Normal, handler);
}
